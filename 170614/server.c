...
int main (int argc, char** argv)
{
	...
	char mese[N], anno[N], localita[N], db_path[N];
	...
	for(;;)
	{
		...
		else if (pid_f == 0)
		{
			...
			if (read(ns, anno, sizeof(anno)-1) < 0)
			{
				perror("read anno");
				exit(11);
			}
			...
			if (read(ns, localita, sizeof(localita)-1) < 0)
			{
				perror("read localita");
				exit(12);
			}
			
			/* Memorizzo il path */
			memset(db_path, 0, sizeof(db_path));
			/* snprintf(db_path, sizeof(db_path)-1, "/var/local/travel/%s.txt", anno); */
			snprintf(db_path, sizeof(db_path)-1, "%s/%s.txt", anno, mese); /* per il test */
			...
			else if (pid_n1 == 0)
			{
				...
				else if (pid_n2 == 0)
				{
					...
					execlp("grep", "grep", localita, db_path, NULL);
					perror("exec grep");
					exit(17);
				}
				/* Nipote N1 */
