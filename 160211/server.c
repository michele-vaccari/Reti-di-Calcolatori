...
int main (int argc, char** argv)
{
	...
	char nomeD[N], nomeC[N], numero[N], db_path[N];
	...
	for(;;)
	{
		...
		else if (pid_f == 0)
		{
			...
			/* Leggi il nome del disegnatore */
			memset(nomeD, 0, sizeof(nomeD));
			if (read(ns, nomeD, sizeof(nomeD)-1) < 0)
			{
				perror("read nomeD");
				exit(9);
			}
			...
			/* Leggi il nome della collana */
			memset(nomeC, 0, sizeof(nomeC));
			if (read(ns, nomeC, sizeof(nomeC)-1) < 0)
			{
				perror("read nomeC");
				exit(11);
			}
			...
			/* snprintf(db_path, sizeof(db_path)-1, "/var/local/comics/%s.txt", nomeC); */
			snprintf(db_path, sizeof(db_path)-1, "%s.txt", nomeC); /* per il test */
			...
			else if (pid_n1 == 0)
			{
				...
				else if (pid_n2 == 0)
				{
					...
					execlp("grep", "grep", nomeD, db_path, NULL);
					perror("exec grep");
					exit(17);
				}
				/* Nipote N1 */
