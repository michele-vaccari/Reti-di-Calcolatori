...
int main (int argc, char** argv)
{
	...
	char nomeP[N], tipoP[N], numero[N], db_path[N];
	...
	for(;;)
	{
		...
		else if (pid_f == 0)
		{
			...
			/* Leggi il nome del progetto */
			memset(nomeP, 0, sizeof(nomeP));
			if (read(ns, nomeP, sizeof(nomeP)-1) < 0)
			{
				perror("read nomeP");
				exit(9);
			}
			...
			/* Leggi il tipo di problema */
			memset(tipoP, 0, sizeof(tipoP));
			if (read(ns, tipoP, sizeof(tipoP)-1) < 0)
			{
				perror("read tipoP");
				exit(11);
			}
			...
			/* snprintf(db_path, sizeof(db_path)-1, "/var/local/issues/%s.txt", nomeP); */
			snprintf(db_path, sizeof(db_path)-1, "%s.txt", nomeP); /* per il test */
			...
			else if (pid_n1 == 0)
			{
				...
				else if (pid_n2 == 0)
				{
					...
					execlp("grep", "grep", tipoP, db_path, NULL);
					perror("exec grep");
					exit(17);
				}
				/* Nipote N1 */
