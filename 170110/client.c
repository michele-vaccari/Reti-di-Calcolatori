...
int main(int argc, char** argv)
{
	...
	int sd, nread;
	char buf[DIM], nomeP[DIM], tipoP[DIM], numero[DIM];
	...
	if (argc != 3)
	{
		printf("Uso: elenca_issue_oss <server> <porta>\n");
		exit(1);
	}
	...
	for(;;) /* Ciclo infinito, 'fine' per uscire*/
	{
		...
		printf("Inserisci il nome del progetto ('fine' per uscire):\n");
		scanf("%s", nomeP); /* ho il terminatore */

		if(strcmp(nomeP, "fine") == 0)
		{
			close(sd);
		...
		printf("Inserisci il tipo di problema ('fine' per uscire):\n");
		scanf("%s", tipoP); /* ho il terminatore */

		if(strcmp(tipoP, "fine") == 0)
		{
			close(sd);
			printf("Hai scelto di terminare il programma.\n");
			break;
		...
		/* Mando il nome del progetto al server */
		if (write(sd, nomeP, strlen(nomeP)) < 0)
		{
			perror("write nomeP");
			exit(4);
		}
		...
		/* Mando il tipo di problema al server */
		if (write(sd, tipoP, strlen(tipoP)) < 0)
		{
			perror("write tipoP");
			exit(7);
		}
