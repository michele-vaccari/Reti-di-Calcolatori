...
int main(int argc, char** argv)
{
	...
	int sd, nread;
	char buf[DIM], argomento[DIM], data[DIM], numero[DIM];
	...
	if (argc != 3)
	{
		printf("Uso: news_browser <server> <porta>\n");
		exit(1);
	}
	...
	for(;;) /* Ciclo infinito, 'fine' per uscire*/
	{
		...
		/* Leggo l'argomento */
		printf("Inserisci l'argomento ('fine' per uscire):\n");
		scanf("%s", argomento); /* ho il terminatore */
		...
		/* Leggo la data */
		printf("Inserisci la data ('fine' per uscire):\n");
		scanf("%s", data); /* ho il terminatore */
		...
		/* Mando l'argomento al server */
		if (write(sd, argomento, strlen(argomento)) < 0)
		{
			perror("write argomento");
			exit(4);
		}
