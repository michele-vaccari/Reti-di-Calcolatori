...
int main (int argc, char** argv)
{
	...
	char nomeDB[N], tipoC[N], sistemaOP[N], db_path[N];
	...
	for(;;)
	{
		...
		else if (pid_f == 0)
		{
			...
			/* Leggi il nome del database */
			memset(nomeDB, 0, sizeof(nomeDB));
			if (read(ns, nomeDB, sizeof(nomeDB)-1) < 0)
			{
				perror("read nomeDB");
				exit(9);
			}
			...
			/* Leggi il tipo del componente */
			memset(tipoC, 0, sizeof(tipoC));
			if (read(ns, tipoC, sizeof(tipoC)-1) < 0)
			{
				perror("read tipoC");
				exit(11);
			}
			...
			/* Leggi il sistema operativo */
			memset(sistemaOP, 0, sizeof(sistemaOP));
			if (read(ns, sistemaOP, sizeof(sistemaOP)-1) < 0)
			{
				perror("read sistemaOP");
				exit(12);
			}
			
			/* Memorizzo il path */
			memset(db_path, 0, sizeof(db_path));
			/* snprintf(db_path, sizeof(db_path)-1, "/var/local/vulnerability_databases/%s.txt", nomeDB); */
			snprintf(db_path, sizeof(db_path)-1, "%s.txt", nomeDB); /* per il test */
			...
			else if (pid_n1 == 0)
			{
				...
				else if (pid_n2 == 0)
				{
					...
					execlp("grep", "grep", tipoC, db_path, NULL);
					perror("exec grep");
					exit(17);
				}
				/* Nipote N1 */
				...
				/* Eseguo la grep e rimando i risultati al figlio */
				execlp("grep", "grep", sistemaOP, NULL);
				perror("exec grep");
				exit(15);
			...
			execlp("sort", "sort", "-rn", NULL); /* sort decrescente */
			perror("exec sort");
			exit(18);
		}
		/* Padre */
