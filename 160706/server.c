...
int main (int argc, char** argv)
{
	...
	char destinazione[N], mese[N], numero[N], db_path[N];
	...
	for(;;)
	{
		...
		else if (pid_f == 0)
		{
			...
			/* Leggi la destinazione */
			memset(destinazione, 0, sizeof(destinazione));
			if (read(ns, destinazione, sizeof(destinazione)-1) < 0)
			{
				perror("read destinazione");
				exit(9);
			}
			...
			if (read(ns, mese, sizeof(mese)-1) < 0)
			{
				perror("read mese");
				exit(11);
			}
			...
			/* snprintf(db_path, sizeof(db_path)-1, "/var/local/travel/%s.txt", mese); */
			snprintf(db_path, sizeof(db_path)-1, "%s.txt", mese); /* per il test */
			...
			else if (pid_n1 == 0)
			{
				...
				else if (pid_n2 == 0)
				{
					...
					execlp("grep", "grep", destinazione, db_path, NULL);
					perror("exec grep");
					exit(17);
				}
				/* Nipote N1 */
				...
				/* Eseguo la tail e rimando i risultati al figlio */
				execlp("tail", "tail", "-n", numero, NULL);
				perror("exec tail");
				exit(15);
