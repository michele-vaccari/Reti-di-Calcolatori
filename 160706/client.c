...
int main(int argc, char** argv)
{
	...
	int sd, nread;
	char buf[DIM], destinazione[DIM], mese[DIM], numero[DIM];
	...
	if (argc != 3)
	{
		printf("Uso: trova_vacanze_last_minute <server> <porta>\n");
		exit(1);
	}
	...
	for(;;) /* Ciclo infinito, 'fine' per uscire*/
	{
		...
		printf("Inserisci la destinazione ('fine' per uscire):\n");
		scanf("%s", destinazione); /* ho il terminatore */

		if(strcmp(destinazione, "fine") == 0)
		{
			close(sd);
		...
		/* Mando la destinazione al server */
		if (write(sd, destinazione, strlen(destinazione)) < 0)
		{
			perror("write destinazione");
			exit(4);
		}
